            ...
            steps {
                ...
                script {
                    if (env.GERRIT_PATCHSET_REVISION) {
                        sh 'FILE=cENM/config/kubesec_config.yaml && git archive --remote=${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.va/oss_va ${GERRIT_PATCHSET_REVISION} "$FILE" | tar -xO "$FILE" >  config/kubesec_config.yaml'
                        ...
                        sh 'FILE=cENM/config/kubesec_config.yaml && git archive --remote=${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.va/oss_va HEAD "$FILE" | tar -xO "$FILE" >  config/kubesec_config.yaml'
                    ...
                    stash includes: "config/", name: "kubesec_config.yaml", allowEmpty: true
                                    ...
                                    script {
                                        if (IMAGES_KUBE.contains(IMAGE_NAME)) {
                                            ...
                                            def ret = sh (
                                                returnStatus: true,
                                                script: '''
                                                    cd reports/'''+IMAGE_NAME+'''/templates
                                                    for file in ./*.json; do
                                                        if [ -s "${file}" ]; then
                                            ...
                                            if (ret == 2) {
                                                ...
                                                writeFile file: "Kubesec_Faulty_Reports.txt", text: kube_faulty_report
                                                archiveArtifacts artifacts: 'Kubesec_Faulty_Reports.txt', allowEmptyArchive: true
                                            ...
                                            writeFile file: "Kubesec_Images_Not_Scanned.txt", text: kube_skipped_images
...
def runScan(String IMAGE, String IMAGE_NAME, String IMAGE_VERSION) {
    withEnv(["IMAGE_NAME=${IMAGE_NAME}", "IMAGE_VERSION=${IMAGE_VERSION}"]) {
        ...
        sh '''
            ...
            mkdir reports
        '''
        unstash "kubesec_config.yaml"
        def HELM_CHART_URL = "https://arm.seli.gic.ericsson.se/artifactory/proj-enm-helm/" + IMAGE_NAME + "/" + IMAGE_NAME + "-" + IMAGE_VERSION + ".tgz"
        sh "sed -i 's#HELM_CHART_URL#${HELM_CHART_URL}#g' config/kubesec_config.yaml"
        generate_report = ci_va_check_report("kubesec")
        if (generate_report) {
            def scan_exit_code = sh (returnStatus: true, script: "${bob} kubesec-scan")
            if (scan_exit_code != 0) {
                currentBuild.result = "FAILURE"
                sh 'exit -1'
            }
            try {
                ci_va_push_report("kubesec", IMAGES_FAIL)
                IMAGES_KUBE_REPORT.add(IMAGE_NAME)
            } catch (Exception e) {
                ...
                archiveArtifacts artifacts: 'Kube_Images_Not_Scanned.txt', allowEmptyArchive: true
